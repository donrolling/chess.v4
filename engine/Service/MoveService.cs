...
namespace chess_engine.Engine.Service
{
	public class MoveService : IMoveService
	{
		...
		public bool IsEnPassant(Square square, int newPiecePosition, string enPassantTargetSquare)
		{
			...
			{ return false; } //if we're not moving to the en passant position, this is not en passant
			var moveDistance = Math.Abs(square.Index - newPiecePosition);
			if (!new List<int> { 7, 9 }.Contains(moveDistance))
			{ return false; } //is this a diagonal move?
			if (piece.Color == Color.Black && square.Index < newPiecePosition)
			{ return false; } //black can't move up
			if (piece.Color == Color.White && square.Index > newPiecePosition)
			{ return false; } //white can't move down
			return true;
		}
