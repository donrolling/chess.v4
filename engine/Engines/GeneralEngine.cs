...
using chess_engine.Engine.Models;
...
namespace chess_engine.Engine.Utility
{
	public static class GeneralEngine
	{
		...
		public static DirectionInfo GetDirectionInfo(int location, int destination)
		{
			...
			if (result.IsOrthogonal)
			{
				...
				if (rankMove)
				{
					result.Direction = destination > location ? Direction.Forward : Direction.Backward;
				} else
				{
					result.Direction = destination > location ? Direction.Right : Direction.Left;
				}
			} else
			{
